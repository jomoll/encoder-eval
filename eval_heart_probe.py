#!/usr/bin/env python3
"""
Evaluate heart/no-heart information in CLIP image embeddings
============================================================

This script loads a (fine-tuned) CLIP model, freezes it, encodes images from the dataset,
and trains a small linear classifier (logistic regression / softmax) on top of the frozen
image embeddings to predict whether a heart is present (H=1) or absent (H=0).

If the classifier achieves AUC >> 0.5 on held-out splits, the encoder retained linearly
decodable information about the silent factor (heart). If AUC ≈ 0.5 (with tight CIs),
it suggests invariance to that factor under your training setup.

What's new in this merged version
---------------------------------
- FIXED accuracy: compute against ground-truth labels (not just fraction predicted positive)
- Adds threshold tuning (on train or eval) with policies: accuracy / Youden's J / F1
- Adds optional temperature scaling (single-scalar) for calibration on train or eval
- Reports: AUC, accuracy@0.5, argmax accuracy, best-threshold accuracy, balanced accuracy, F1, chosen threshold & temperature
- Keeps your mode-specific save directory logic and masking audit
- SUPPORTS SmallResNetCLIP models with custom loading

Usage
-----
pip install torch torchvision transformers datasets pillow tqdm scikit-learn

# Example: standard-CLIP baseline
python eval_heart_probe.py \
  --dataset_id jomoll/silent-heart-dataset \
  --model_path ./ckpt_scratch_standard/best \
  --split_train train --split_eval val \
  --epochs 20 --lr 5e-3 \
  --tune_threshold_on eval --tune_policy acc --calibrate_on train

# Example: small ResNet model
python eval_heart_probe.py \
  --dataset_id jomoll/silent-heart-dataset \
  --model_path ./ckpt_small_resnet/best \
  --split_train train --split_eval val \
  --mask_special --tune_threshold_on eval --tune_policy youden
"""
...
from train_clip_modes import SmallResNetCLIP, SmallResNetVision 
...
def load_model(model_path: str, device):
    """Load either standard CLIP or custom SmallResNetCLIP model"""
...
def mask_special_region(img: Image.Image, ex: Dict[str, Any]) -> Image.Image:
    """
    Black out a circular region around the special object's center.
...
def build_dataset_with_labels(dataset_id: str,
    ...
    # helper: resolve label H
    def resolve_label(ex):
        # preferred: explicit column H
        if "H" in ex:
            v = ex["H"]
            # could be int or string label ("decoy","heart")
            if isinstance(v, int):
                return v
            try:
                # ClassLabel -> string
                ...
                # datasets ClassLabel type may already be int
                return int(v)
            except Exception:
                pass
        # fallback: parse from id suffix ..._<H>
        if "id" in ex and isinstance(ex["id"], str) and ex["id"][-2:] in ["_0","_1"]:
            return int(ex["id"][-1])
        # last resort: check metadata flag in JSON fields if exist
    ...
    def _transform(examples):
        ...
        if is_batched:
            ...
            for i, img in enumerate(images):
                # Get single example for this index
                ...
                labels.append(resolve_label(ex_single))
            ...
            examples["label"] = resolve_label(examples)
...
def embed_split(model, loader: DataLoader, device, amp_dtype):
    model.eval()
    feats, labels = [], []
    ...
    with torch.no_grad():
        for batch in tqdm(loader, desc="Embedding"):
            ...
            with autocast(dtype=amp_dtype):
                # Use get_image_features for normalized projected embeddings
...
def pick_threshold(y_true: np.ndarray, scores: np.ndarray, policy: str = "acc"):
    """
    Pick a decision threshold on 'scores' for positive class.
    policy: 'acc' (max accuracy), 'youden' (maximize TPR-FPR), 'f1' (max F1)
    """
...
class TemperatureScaler(nn.Module):
    """
    Single-scalar temperature scaling for logits calibration.
    """
    def __init__(self):
        super().__init__()
...
def calibrate_temperature(logits: torch.Tensor, labels: torch.Tensor, max_iter=200, lr=0.05):
    """
    Fit a temperature scalar by minimizing NLL on provided logits/labels.
    """
...
def main():
    ...
    ap.add_argument("--model_path", type=str, required=True, help="Path or HF id for the fine-tuned CLIP model (directory with config)")
    ...
    ap.add_argument("--mask_special", action="store_true", help="Mask out the special object's region (proxy audit)")
    ...
    # Extract mode from model_path for save directory
    model_path_parts = args.model_path.replace("\\", "/").split("/")
    mode_name = "unknown"
    for part in model_path_parts:
        if part.startswith("ckpt_"):
            mode_name = part.replace("ckpt_", "")
            break
    
    # Create mode-specific save directory
    mode_save_dir = os.path.join(args.save_dir, mode_name)
    ...
    # Load model + processor (now supports custom models)
    model = load_model(args.model_path, device)
    model.eval()
    for p in model.parameters():
        p.requires_grad_(False)
    
    # Load image processor - try from model path first, fall back to default
    try:
        processor = CLIPImageProcessor.from_pretrained(args.model_path)
    except:
        print("Could not load image processor from model path, using default")
        processor = CLIPImageProcessor.from_pretrained("openai/clip-vit-base-patch32")

    # Build datasets and loaders
    ds_train = build_dataset_with_labels(args.dataset_id, processor, split=args.split_train, mask_special=args.mask_special)
    ds_eval  = build_dataset_with_labels(args.dataset_id, processor, split=args.split_eval,  mask_special=args.mask_special)
    ...
    # Embed
    Xtr, ytr = embed_split(model, dl_train, device, amp_dtype)
    Xev, yev = embed_split(model, dl_eval, device, amp_dtype)

    # Convert embeddings to float32 to match probe weights
    ...
    model_type = "small_resnet" if isinstance(model, SmallResNetCLIP) else "clip"
